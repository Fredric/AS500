 ...
 */
function writeToGrid(grid: string[][], row: number, col: number, content: string): void {
...
export function render(
  ...
  let defaultCursorField: string | undefined = screen.defaultCursor;
  ...
  for (const element of screen.elements) {
    switch (element.kind) {
      ...
      case 'subfile': {
        const pageOffset = (context[`${element.name}_offset`] as number) || 0;
 ...
 */
export function defineScreen(id: string, config: Omit<ScreenDef, 'id'>): ScreenDef {
  return {
    id,
    ...config,
  };
