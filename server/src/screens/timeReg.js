...
import { defineScreen, render, header, text, subfile, } from '../dsl/index.js';
import { getOrCreateDay, getDayItems, deleteDayItem, getDayName, formatHours, getPreviousDay, getNextDay, } from '../services/timeReg.js';
// ============================================
...
// Screen Builder
// ============================================
export function buildTimeRegScreen(session, message = null, messageType = null) {
    const userId = session.viserId;
    // Get current date from context or use today
    const currentDate = session.context.timeRegDate || new Date().toISOString().split('T')[0];
...
// Screen Handler
// ============================================
export function handleTimeReg(session, request) {
    ...
    // F7 - Previous day
    if (request.key === 'F7') {
        const currentDate = session.context.timeRegDate;
    ...
    if (request.key === 'ENTER') {
        const dayId = session.context.timeRegDayId;
