...
import { listBackups, createBackup, formatSize, BackupInfo } from '../services/backup.js';
...
export function buildBackupMgmtScreen(
  session: Session,
  message?: string,
  ...
  const backupsData = backupList.map((backup: BackupInfo) => ({
