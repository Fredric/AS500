...
import type { SubfileElement, SubfileColumnDef, FieldType } from '../types.js';

export interface SubfileOptions {
  showMore?: boolean;  // Show "More..." indicator when there are more records
}
 ...
 */
export function renderSubfile(
  element: SubfileElement,
  ...
  const data = (context[element.name] as Array<Record<string, unknown>>) || [];
  ...
  let headerContent = '';
  let currentCol = 0;
  const columnPositions: Array<{ col: number; width: number }> = [];
 ...
 */
export function getFirstSubfileField(element: SubfileElement): string | undefined {
