...
// ---------------------------------------------------------------------------
// Authenticated user — resolved once at startup from AS500_USERNAME
// ---------------------------------------------------------------------------
...
async function resolveConfiguredUser(): Promise<AuthUser> {
  const username = process.env.AS500_USERNAME?.toUpperCase().trim();
  if (!username) throw new Error("AS500_USERNAME environment variable is not set.");
  ...
  if (!rows[0]) throw new Error(`User '${username}' not found or inactive in AS500.`);
  return rows[0];
}
...
async function getOrCreateDay(userId: number, workday: string): Promise<{ id: number; daysum: string }> {
...
async function refreshDaysum(dayId: number): Promise<void> {
  await db
    .update(days)
...
Returns JSON of the updated entry.`,
  ...
  async ({ entry_id, start_hour, end_hour, jiratask, description }) => {
    try {
      ...
      if (!entryRows[0]) return errorResponse(`No time entry found with id ${entry_id}.`);
...
Use the 'id' value (e.g. "TASK-101") as jiratask when creating or updating time entries.`,
  ...
  async () => {
    try {
      ...
      if (!apiToken) return errorResponse("JIRA_API_TOKEN environment variable is not set.");
      if (!jiraEmail) return errorResponse("JIRA_USER_EMAIL environment variable is not set.");
      ...
      const tasks = response.data.issues.map((issue: Record<string, any>) => ({
        id: issue.key,
